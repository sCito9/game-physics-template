...
void Scene1::onDraw(Renderer& renderer){
    ...
    for (auto& particle : particles){
        renderer.drawSphere(particle.position, 0.1f, glm::vec4(cmap(particle.lifetime / particle_lifetime), 1));
    }
...
void Scene1::onGUI()
{
    ...
    ImGui::SliderFloat("Particle Lifetime", &particle_lifetime, 0.5f, 3.f);
...
void Scene1::simulateStep(){
    ...
    particles.erase(std::remove_if(particles.begin(), particles.end(), [this](const Particle& particle){
    return particle.lifetime > particle_lifetime;
}), particles.end());
